...
function parseJsonRobust(text: string): unknown {
  ...
  try {
    ...
    // Strip unescaped ASCII control characters (common Gemini artifact)
    const repaired = slice.replace(/[\x00-\x08\x0B\x0C\x0E-\x1F]/g, "");
...
For each issue, be specific: quote the weak passage or name the missing section. Do not say "add more detail" - say exactly what detail is missing and why it matters.

IMPORTANT: All string values must use only standard ASCII characters. Do not use special quotes, em dashes, or Unicode characters in your JSON output.
