...
import { buildEeatPrompt, EEAT_SYSTEM } from "@/lib/blog-gen/prompts";
...
export async function POST(req: NextRequest) {
  ...
  const stream = new ReadableStream({
    async start(streamController) {
      ...
      try {
        send({ type: "progress", step: "Running E-E-A-T pass (Claude Sonnet 5)…" });
        ...
        send({ type: "result", phase: "polish", data: eeat });
      } catch (err) {
