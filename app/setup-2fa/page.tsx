...
function SetupContent() {
  ...
  useEffect(() => {
    if (!token) {
      setError("Missing setup token. Add ?token=YOUR_TOTP_SETUP_TOKEN to the URL.");
      return;
    }
  ...
  return (
    <div className="w-full max-w-md rounded-xl border border-[var(--border)] bg-[var(--card)] p-8 shadow-sm">
      <div className="mb-1 flex items-center gap-2">
        <span className="flex h-8 w-8 items-center justify-center rounded-lg bg-[var(--accent-soft)] text-[var(--accent)] text-sm font-bold">R</span>
        <h1 className="text-lg font-semibold text-[var(--foreground)]">Set up 2FA</h1>
      </div>
      <p className="mb-6 text-sm text-[var(--muted)]">Scan the QR code with Google Authenticator.</p>
      ...
      {data && (
        <div className="space-y-6">
          <div className="flex justify-center">
            ...
            <img src={data.qrCodeUrl} alt="TOTP QR Code" width={256} height={256} className="rounded-lg border border-[var(--border)]" />
          </div>

          <div>
            <p className="mb-1.5 text-xs font-medium text-[var(--muted)]">Can&apos;t scan? Enter this secret manually:</p>
            <div className="flex items-center gap-2">
              ...
              <button onClick={copySecret} className="shrink-0 rounded-lg border border-[var(--border)] px-3 py-2 text-xs font-medium text-[var(--muted)] hover:text-[var(--foreground)] transition">
                {copied ? "Copied!" : "Copy"}
              </button>
            </div>
          </div>
          ...
          <a href="/login" className="block w-full rounded-lg bg-[var(--accent)] py-2.5 text-center text-sm font-medium text-white transition hover:bg-blue-700">
            Go to sign in
          </a>
        </div>
      )}
